...
## 구현된 것
- 닉네임 + 팀 선택 후 입장 (`/`)
- **ROUND 1** (`/round1`): 5×5 밸런스 빙고, 칸마다 A/B 선택 · 제출 (팀 점수는 완주자 개인 점수의 평균)
...
- **ROUND 2**
  ...
  - `/round2/guess`: 팀원 중 한 명이 대표자 답을 예측해서 팀당 한 번 제출
...
이어서 `supabase/schema_v11.sql`을 실행합니다. ROUND 1 팀 점수를 완주한 팀원들의
개인 점수 평균으로 저장할 수 있도록 소수 둘째 자리까지 허용합니다.

이어서 `supabase/schema_v12.sql`을 실행합니다. ROUND 2 일반 빙고 제출을 참가자별이
아닌 팀별 한 번으로 제한합니다.
...
3. 참가자들은 `/round2`에서 대표자 1명만 "저는 대표자예요" 클릭 → `/round2/answer`에서 정답 제출
   팀원 중 한 명은 "저는 예측할게요" → `/round2/guess`에서 팀당 한 번 예측 제출
