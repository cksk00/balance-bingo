  ...
  return (
    <section className="mb-6 rounded-2xl bg-white p-5 shadow">
      <div className="flex items-center justify-between gap-4">
        <div><p className="text-sm text-navy/60">{round ? `ROUND ${round} 실시간 입장 현황` : "실시간 입장 현황"}</p>{round ? <p className="mt-1 text-navy"><strong className="text-3xl">{players.filter((player) => player.current_round === round).length}</strong>명</p> : <div className="mt-1 flex flex-wrap gap-x-4 gap-y-1 text-navy"><p><strong className="text-2xl">{players.filter((player) => player.current_round === 1).length}</strong>명 <span className="text-sm text-navy/60">ROUND 1</span></p><p><strong className="text-2xl">{players.filter((player) => player.current_round === 2).length}</strong>명 <span className="text-sm text-navy/60">ROUND 2</span></p><p className="self-end text-sm text-navy/50">전체 {players.length}명</p></div>}</div>
      ...
      {open && <div className="mt-5 space-y-5">{(round ? [round] : [1, 2] as const).map((shownRound) => <div key={shownRound}><h3 className="mb-2 font-extrabold text-navy">ROUND {shownRound} 입장 · {players.filter((player) => player.current_round === shownRound).length}명</h3><div className="grid gap-3 sm:grid-cols-2">{TEAMS.map((team) => { const members = players.filter((player) => player.team_id === team.id && player.current_round === shownRound); return <div key={team.id} className="rounded-xl bg-blue-50 p-3"><p className="font-extrabold text-navy">{team.name} <span className="text-sm text-navy/50">{members.length}명</span></p><p className="mt-1 text-sm text-navy/70">{members.length ? members.map((member) => member.nickname).join(", ") : "없음"}</p></div>; })}</div></div>)}</div>}
    </section>
  );
}
