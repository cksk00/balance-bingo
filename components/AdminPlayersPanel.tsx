...
export function AdminPlayersPanel({ round }: { round?: 1 | 2 }) {
  ...
  const activePlayers = players.filter((player) => activeSessionIds.has(player.id));
  ...
  const refresh = useCallback(async () => {
    const [{ data, error }, sessionsResponse] = await Promise.all([
      supabase.from("players").select("id, nickname, team_id, current_round").order("created_at"),
      fetch("/api/admin/sessions", { cache: "no-store" }),
    ]);
    const sessionsResult = await sessionsResponse.json().catch(() => ({}));
    if (!error && sessionsResponse.ok) {
      setPlayers((data || []) as Player[]);
      setActiveSessionIds(new Set((sessionsResult.playerIds || []) as string[]));
      setError("");
    } else {
      setError(`입장 현황 조회 실패: ${error?.message || sessionsResult.error || "알 수 없는 오류"}`);
    }
  }, []);
  ...
  async function clearSession(player: Player) {
    ...
    if (!response.ok) setError(result.error || "세션 삭제에 실패했습니다.");
      ...
      setNotice(`${player.nickname}님의 세션을 삭제했습니다. 답안과 점수는 유지됩니다.`);
      await refresh();
    }
  ...
  return (
    <section className="mb-6 rounded-2xl bg-white p-5 shadow">
      <div className="flex items-center justify-between gap-4">
        <div><p className="text-sm text-navy/60">{round ? `ROUND ${round} 실시간 입장 현황` : "실시간 입장 현황"}</p>{round ? <p className="mt-1 text-navy"><strong className="text-3xl">{activePlayers.filter((player) => enteredRound(player, round)).length}</strong>명</p> : <div className="mt-1 flex flex-wrap gap-x-4 gap-y-1 text-navy"><p><strong className="text-2xl">{activePlayers.filter((player) => enteredRound(player, 1)).length}</strong>명 <span className="text-sm text-navy/60">ROUND 1</span></p><p><strong className="text-2xl">{activePlayers.filter((player) => enteredRound(player, 2)).length}</strong>명 <span className="text-sm text-navy/60">ROUND 2</span></p><p className="self-end text-sm text-navy/50">활성 {activePlayers.length}명</p></div>}</div>
      ...
      {open && <div className="mt-5 space-y-5">{(round ? [round] : [1, 2] as const).map((shownRound) => <div key={shownRound}><h3 className="mb-2 font-extrabold text-navy">ROUND {shownRound} 입장 · {activePlayers.filter((player) => enteredRound(player, shownRound)).length}명</h3><div className="grid gap-3 sm:grid-cols-2">{TEAMS.map((team) => { const members = activePlayers.filter((player) => player.team_id === team.id && enteredRound(player, shownRound)); return <div key={team.id} className="rounded-xl bg-blue-50 p-3"><p className="font-extrabold text-navy">{team.name} <span className="text-sm text-navy/50">{members.length}명</span></p>{members.length ? <div className="mt-2 space-y-1.5">{members.map((member) => <div key={member.id} className="flex items-center justify-between gap-2 rounded-lg bg-white/70 px-2 py-1.5"><span className="text-sm font-semibold text-navy/70">{member.nickname}</span><button onClick={() => clearSession(member)} disabled={clearingSession === member.id} className="rounded-md bg-red-100 px-2 py-1 text-[11px] font-bold text-red-700 disabled:opacity-40">{clearingSession === member.id ? "삭제 중" : "세션 삭제"}</button></div>)}</div> : <p className="mt-1 text-sm text-navy/70">없음</p>}</div>; })}</div></div>)}</div>}
    </section>
  );
}
