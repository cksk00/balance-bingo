    ...
    return (
      <section className="mx-auto w-full max-w-xl rounded-3xl bg-navy p-6 text-white shadow-xl sm:p-9">
        ...
        <div className="grid gap-4 sm:grid-cols-[1fr_auto_1fr] sm:items-stretch">
          <button key={`option-a-${step}`} onClick={() => choose("A")} className={`min-h-32 rounded-2xl p-5 text-lg font-extrabold transition active:scale-[0.98] sm:min-h-52 ${selected === "A" ? "bg-accentA ring-4 ring-white" : "bg-white/10"}`}>{cell.option_a}</button>
          <span className="self-center text-center text-sm font-black text-hit">VS</span>
          <button key={`option-b-${step}`} onClick={() => choose("B")} className={`min-h-32 rounded-2xl p-5 text-lg font-extrabold transition active:scale-[0.98] sm:min-h-52 ${selected === "B" ? "bg-accentB ring-4 ring-white" : "bg-white/10"}`}>{cell.option_b}</button>
        </div>
